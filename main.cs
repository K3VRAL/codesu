...
namespace osuProgram
{
    public partial class Program
    {
        public static void Main(string[] args)
        {
            ...
            {
                string file = null;
                bool debug = false, ignore = false, step = false, all = false;
                ...
                else if (osufile[osufile.Length - 1] != "osu")
                {
                ...
                while (line < AllHitObjects.Count)
                {
                    ...
                    {
                        // Jump
                        // TODO: Make sure everything has been correctly implemented - so far so good
                        // TODO: Make it count the brackets precompiling and give an error if the brackets do not have the value of 0
                        int bracketcount = 0;
                        switch (AllHitObjects[line].OType)
                        {
                            case GetObjectInfo.Type.Normal:
                                for (int i = line; i < AllHitObjects.Count; i++)
                                {
            ...
            {
                Console.WriteLine("Error: args: {0}", args);
            }
            return;
        }
    }
